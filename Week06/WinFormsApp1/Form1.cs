...
namespace WinFormsApp1
{
    ...
    public class OthelloForm : Form
    {
        ...
        public class Board
        {
            ...
            public class Player
            {
                private string m_Sign; // The player's sign, e.g., "X" or "O"
                ...
                public string GetOpponentSign()
                {
                    ...
                    {
                        // Handle the case where the sign is neither "X" nor "O"
                        return "";
                    }
                }
            }
        ...
        private void InitializeComponents()
        {
            ...
            for (int row = 0; row < BoardSize; row++)
            {
                for (int col = 0; col < BoardSize; col++)
                {
                    ...
                    cellButton.Click += CellButtonClick;
                    Controls.Add(cellButton);
                }
            }

            // Other UI elements, labels, and buttons can be added here
        }

        private void CellButtonClick(object sender, EventArgs e)
        {
            Button cellButton = (Button)sender;
            int row = ((ValueTuple<int, int>)cellButton.Tag).Item1;
            int col = ((ValueTuple<int, int>)cellButton.Tag).Item2;

            // Check if it's a valid move
            if (gameBoard.IsValidMove(new Board.Player("X"), row, col))
            {
                // Update the game board and UI
                gameBoard.UpdateBoardState(new Board.Player("X"), row, col);
                UpdateBoardUI();

                // Implement logic to switch to the next player's turn
            }
            else
            {
        ...
        {
            // Update the appearance of the buttons to reflect the game state
            for (int row = 0; row < BoardSize; row++)
            {
                for (int col = 0; col < BoardSize; col++)
                {
                    Button cellButton = Controls.OfType<Button>()
                        .Where(btn => ((ValueTuple<int, int>)btn.Tag).Item1 == row && ((ValueTuple<int, int>)btn.Tag).Item2 == col)
                        .FirstOrDefault();

                    if (cellButton != null)
                    {
                        // Update the appearance of the button based on the game board state
                        string cellState = gameBoard.ScreenBoard[row, col];
                        cellButton.Text = cellState;
                    }
                }
            }
        }
